...
public: 
    ...
    void give_back() {
        if (this->owner != nullptr) { 
            if (this->mut_borrower == nullptr && this->borrowers.size() == 0) { 
                ...
                // changing owner value if was borrowed as mutable
                if (this->MUT_FLAG) { this->owner->value = this->value; } // ERROR
                this->owner = nullptr;
                this->value = nullptr;
            }
    ...
    void debug() {
        if (this->value == nullptr) { std::cout << "nptr "; }
        else { std::cout << *this->value << " "; }

        for (auto e : this->borrowers) {
            std::cout << e << ":";
        }
        if (this->borrowers.size() == 0) { std::cout << "-"; }
        std::cout << " ";
        
        if (this->mut_borrower == nullptr) { std::cout << " nptr "; }
        else { std::cout << this->mut_borrower; }
        
        std::cout << "\n";

        std::cout << this << " ";

        if (this->owner == nullptr) { std::cout << " nptr "; }
        else { std::cout << this->owner; }
